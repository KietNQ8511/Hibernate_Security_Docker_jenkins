...
public class UserControllerTest {
	...
	void findByUserName_NotFound() {
		...
		ResponseEntity<User> response = userController.findByUserName("kietnq");
	...
	void createUser_invalidConfirmPassword() {
		...
		CreateUserRequest request = createUserRequest();
		request.setPassword("kietpass");
		request.setConfirmPassword("kietpass2");
		ResponseEntity<User> response = userController.createUser(request);
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
	}
	...
	void createUser_Success() {
		...
		CreateUserRequest request = createUserRequest();
		request.setPassword("kietpass");
		request.setConfirmPassword("kietpass");
		ResponseEntity<User> response = userController.createUser(request);
		assertEquals(HttpStatus.OK, response.getStatusCode());
	}
